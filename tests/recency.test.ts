...
describe("deterministic configured article recency", () => {
    ...
    it("does not reuse a v0.5.0 persistent allow after the stricter-rule instructions change", async () => {
        const directory = await mkdtemp(join(tmpdir(), "moderation-legacy-recency-"));
        dirs.push(directory);
        const cachePath = join(directory, "cache.json");
        // Generated by the published v0.5.0 bundle for the exact settings/community/publication below.
        // That version's cache identity omitted the user instructions that incorrectly waived stricter rules.
        const legacyKey = "625238e7eccb70796c4c775fa9db4902b8e130bd680b8fa52473367fc122eae7";
        await writeFile(
            cachePath,
            JSON.stringify({
                version: 1,
                entries: {
                    [legacyKey]: {
                        cachedAt: Date.now(),
                        verdict: { verdict: "allow", reason: "", matchedRuleIndexes: [] },
                        providerModel: "model-test",
        ...
        const fetch = stub("review");
        const publication = request("2026/09/15", { title: "Legacy configured-recency cache regression" });
        expect(await evaluate({ articleMaxAgeHours: "240", cachePath }, publication)).toMatchObject({ success: false });
        expect(fetch).toHaveBeenCalledTimes(1);
        const body = JSON.parse(String(fetch.mock.calls[0]?.[1]?.body));
        expect(JSON.parse(body.input[1].content).instructions).toContain("including any stricter explicit article-age limit");
        const cache = JSON.parse(await readFile(cachePath, "utf8"));
        expect(Object.keys(cache.entries)).toHaveLength(2);
        expect(cache.entries[legacyKey].verdict.verdict).toBe("allow");
    });
