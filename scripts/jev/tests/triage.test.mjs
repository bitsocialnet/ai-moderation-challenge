...
const answer = (question, choice, p = 1) => ({
    ...
    probabilities: Object.fromEntries(
        Object.keys(question.criteria).map((key) => [key, key === choice ? p : (1 - p) / (Object.keys(question.criteria).length - 1)])
    )
});
const clientFor = (severity = "error", action = "inspect_dependency", p = 1) => {
    ...
    return {
        ask: async () => {
            ...
            return {
                model: "jev-1.13.0",
                answers: { severity: answer(triageQuestions.severity, severity, p), action: answer(triageQuestions.action, action, p) },
                latencyMs: 5
            };
        },
...
test("committed corpus is explicitly synthetic and offline never calls the client", async () => {
    const fixture = JSON.parse(await readFile(new URL("../fixtures/triage.json", import.meta.url), "utf8"));
    const report = await triage({
        input: fixture,
        client: { ask: () => assert.fail("offline request"), stats: () => assert.fail("offline stats") }
    });
...
test("offline CLI ignores unavailable private config and does not output selected message text", () => {
    const run = spawnSync(
        process.execPath,
        [new URL("../triage.mjs", import.meta.url).pathname, "--input", new URL("../fixtures/triage.json", import.meta.url).pathname],
        {
            encoding: "utf8",
            env: { ...process.env, JEV_CONFIG_FILE: "/nonexistent/jev-fixture-only.json", TYPESAFE_API_KEY: "", JEV_MODEL: "" }
        }
    );
...
test("exact repeats group once with occurrence counts; different source/text stay separate", async () => {
    const selected = {
        ...
        events: [
            { ...event, count: 3 },
            { ...event, id: "repeat", count: 4, expected: { action: "inspect_dependency", severity: "error" } },
            { ...event, id: "other-source", source: "moderation" },
            { ...event, id: "other-message", message: "Required RPC connection reset; indexing remains stopped." }
        ]
    };
...
test("rejects raw fields, unlabeled real data with manufactured expectations, conflicting labels, and bounds", () => {
    for (const invalid of [
        ...
        { ...input, events: [event, { ...event, id: "conflict", expected: { severity: "critical", action: "security_review" } }] },
        { ...input, events: Array.from({ length: 21 }, (_, i) => ({ ...event, id: `case-${i}`, message: `Failure ${i}` })) }
    ])
...
test("wrong dismissals are measured separately from abstention and ordinary category mismatch", async () => {
    const dismissed = await triage({ input, live: true, client: clientFor("informational", "observe") });
    ...
    const wrongCategory = await triage({ input, live: true, client: clientFor("error", "inspect_application") });
...
test("actual client rejects unknown choice and enforces request budget without duplicate calls", async () => {
    ...
    const report = await triage({
        input: { ...input, events: [event, { ...event, id: "second", message: "Another selected error." }] },
        live: true,
        client
    });
...
test("CLI failure is generic and private input never appears in output", async () => {
    ...
    try {
        ...
        await writeFile(file, JSON.stringify({ ...input, events: [{ ...event, message: "password: PRIVATE_TEST_VALUE" }] }));
        const run = spawnSync(process.execPath, [new URL("../triage.mjs", import.meta.url).pathname, "--input", file, "--live"], {
            encoding: "utf8"
        });
