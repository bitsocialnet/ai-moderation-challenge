...
test("related groups remain in one split, duplicate items rejected", () => {
    ...
    assert.throws(
        () => create([item(), item({ content: { ...item().content, source: "Other source" }, split: "holdout" })]),
        /cross frozen splits/
    );
});
...
test("rendered page exports importer-compatible answers and safely restores a draft", async () => {
    ...
    const ids = Object.fromEntries(
        ["items", "reviewer", "date", "independent", "download", "message", "resume"].map((id) => [id, new Element(id)])
    );
    let downloaded;
