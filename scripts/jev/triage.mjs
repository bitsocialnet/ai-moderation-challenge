...
const actions = ["observe", "inspect_dependency", "inspect_configuration", "inspect_application", "security_review", "uncertain"];
const sources = ["moderation", "indexer", "spam-blocker", "archive-web"];
const judgmentPolicy = { minimumProbability: 0.8, minimumConfidence: 0.6, observeContradictions: ["error", "critical"] };
const fail = () => {
...
export function prepareGroups({ input }) {
    ...
    for (const event of input.events) {
        ...
        if (!groups.has(groupId))
            groups.set(groupId, { groupId, source: event.source, message, occurrences: 0, selectedEvents: 0, expected: event.expected });
        const group = groups.get(groupId);
        if (group.expected?.severity !== event.expected?.severity || group.expected?.action !== event.expected?.action) fail();
...
export const triageQuestions = {
    severity: {
        ...
        criteria: {
            informational: "Routine successful operation or explicitly expected idle state; no failure is reported.",
            warning: "An explicitly recovered transient failure or degraded optional feature; the core operation is working.",
            error: "An operation or required dependency is failing, and needs investigation; no confirmed critical harm is stated.",
            critical:
    ...
    action: {
        ...
        criteria: {
            ...
            inspect_dependency: "Evidence points to provider, RPC, storage, database, or upstream availability/response problems.",
            inspect_configuration: "Evidence identifies a missing, invalid, or incompatible setting/input configuration.",
            inspect_application: "Evidence identifies application parsing, logic, or lifecycle behavior requiring code investigation.",
            security_review: "Evidence identifies credential exposure, unauthorized access, or a security control bypass.",
...
function summarize({ groups, rows, provenance }) {
    ...
    for (const group of labeled) {
        ...
        if (
            (["error", "critical"].includes(group.expected.severity) || !["observe", "uncertain"].includes(group.expected.action)) &&
...
export async function triage({ input, live = false, client, maxRequests = 20, maxCostUsd = 0.01 }) {
    ...
    const rows = [],
        helper = live
            ? (client ??
              createJevClient({ live: true, maxRequests, maxCostUsd, maxInputBytes: 12000, maxInputTokens: 200000, deadlineMs: 120000 }))
            : null;
    for (const group of groups) {
        ...
        if (live) {
            try {
                ...
                if (typeof result.model !== "string" || !/^jev-\d+\.\d+\.\d+$/.test(result.model)) throw new JevError("invalid_response");
                ...
                const uncertain =
                    ...
                    (judgmentPolicy.observeContradictions.includes(severity.choice) && action.choice === "observe");
...
export async function triageMain({ args }) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        else if (["--input", "--max-requests", "--max-cost-usd"].includes(key) && args[i + 1]) flags[key] = args[++i];
        else fail();
    }
    if (!flags["--input"] || (!flags["--live"] && (flags["--max-requests"] || flags["--max-cost-usd"]))) fail();
    ...
    if (result.mode === "live") process.exitCode = result.metrics.unverifiedGroups ? 2 : result.metrics.mismatches ? 1 : 0;
}
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
    triageMain({ args: process.argv.slice(2) }).catch(() => {
        console.log(JSON.stringify({ version: 1, status: "unverified", reason: "invalid_or_sensitive_input", advisoryOnly: true }));
        process.exitCode = 2;
    });
}
