...
Moderation exposes provider attempts and `moderation_error` in its private audit, plus audit-write errors in `src/index.ts`. The raw audit contains publications and must never be supplied here. Select an operator-written sanitized failure summary instead. Keep `scripts/moderation-usage.mjs` authoritative for deterministic provider/cache/token accounting; this helper only adds an advisory semantic classification.
