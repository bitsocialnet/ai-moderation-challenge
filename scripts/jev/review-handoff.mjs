    ...
    const rows = items.map((item) => {
        ...
        if (kind === "translation") {
            ...
            if (source.kind !== "synthetic")
                fail("Moderation handoff currently accepts explicitly synthetic fixtures only; no private traffic importer");
            if (
            ...
            if (
                ...
                (p.kind === "commentEdit" &&
                    (typeof p.content !== "string" || ["title", "link", "linkHtmlTagName"].some((key) => p[key] !== undefined))) ||
...
export function validateReviewQueue(queue) {
    ...
    if (queue.items.some((item) => !exact(item, ["id", "contentSha256", "group", "split", "source", "content"])))
        fail("Unexpected review item fields");
    const { sha256: _rubricHash, ...rubric } = queue.rubric;
    const rebuilt = createReviewQueue({
        ...queue,
        rubric,
        items: queue.items.map(({ content, source, group, split }) => ({ content, source, group, split }))
    });
