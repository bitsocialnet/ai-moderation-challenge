    ...
    for (const rule of input.rules) {
        if (
            ...
            rule.extensions.some(
                (x) => ![".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".json", ".md", ".mdx", ".html", ".css"].includes(x)
            )
        )
            fail("invalid_review_rules");
        seen.add(rule.id);
...
export function validateReviewInput(input, rules) {
    ...
    if (input.provenance !== undefined && !["synthetic", "human-reviewed", "unspecified"].includes(input.provenance))
        ...
        if (
            ...
            (!object(item.expected) ||
                ...
                Object.entries(item.expected).some(
                    ([id, value]) => !rules.some((rule) => rule.id === id) || !["clear", "issue", "uncertain"].includes(value)
                ))
        )
...
export async function main(argv = process.argv.slice(2)) {
    ...
    if (Boolean(values.input) === Boolean(values.base) || (values.input && values.files) || (values.base && !values.files))
